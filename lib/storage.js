...
export function loadState(config = {}) {
  ...
  const finalState = normalizeState(normalizedState);

  // Supabase is the authoritative source for worked dates.
  // Any dates previously stored locally are queued as pending claims so they
  // get pushed to Supabase on the next sync (handles one-time migration too).
  finalState.pendingClaims = uniqueDateStrings([
    ...finalState.pendingClaims,
    ...finalState.workedDates,
  ]);
  finalState.workedDates = [];
...
export function saveState(state) {
  ...
  // Do not persist workedDates — Supabase is the source of truth.
  // Only settings, pendingClaims, ui, sync meta, etc. are stored locally.
  const { workedDates: _omitted, ...stateToSave } = normalizedState;
